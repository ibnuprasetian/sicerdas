    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div class="bg-white rounded-xl shadow-md overflow-hidden animate-slide-in-left">
            <div class="p-6">
                ...
                <div id="avatar-preview" class="relative w-24 h-24 mx-auto rounded-full <?php echo e($bgColor); ?> flex items-center justify-center text-white text-4xl shadow-lg transition-transform duration-300 hover:scale-105">
                    <i class="fas fa-<?php echo e($icon); ?> animate-pulse"></i>
                ...
                <div id="avatar-options" class="hidden absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 mt-4 w-4/5 md:w-1/2 bg-white p-6 rounded-xl shadow-lg border border-gray-200 z-20 animate-fade-in">
                    ...
                    <div class="grid grid-cols-4 gap-4 justify-center">
                        <?php $__currentLoopData = $colorMap; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $color): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <label class="cursor-pointer transition-transform duration-200 hover:scale-110">
                                <input type="radio" name="photo" value="<?php echo e($val); ?>" class="hidden" form="profile-form" <?php echo e($icon == $val ? 'checked' : ''); ?>>
                                <span class="w-12 h-12 rounded-full <?php echo e($color); ?> flex items-center justify-center text-white text-2xl shadow-md icon-option" data-icon="<?php echo e($val); ?>" data-color="<?php echo e($color); ?>">
                                    <i class="fas fa-<?php echo e($val); ?>"></i>
                                </span>
                            </label>
                ...
                <div class="text-center mt-6 space-y-2">
                    <p class="text-lg font-semibold text-gray-800"><?php echo e(Auth::user()->name); ?></p>
                    <p class="text-gray-600"><?php echo e(Auth::user()->email); ?></p>
                    <p>
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium <?php echo e(Auth::user()->role == 1 ? 'bg-green-100 text-green-800' : 'bg-purple-100 text-purple-800'); ?>">
                            <i class="fas fa-<?php echo e(Auth::user()->role == 1 ? 'user-tie' : 'user-graduate'); ?> mr-1"></i>
                            <?php echo e(Auth::user()->role == 1 ? 'Guru' : 'Siswa'); ?>
        ...
        <div class="md:col-span-2 bg-white rounded-xl shadow-md overflow-hidden animate-slide-in-right">
            <div class="p-6">
                ...
                <form id="profile-form" method="POST" action="<?php echo e(route('profile.update')); ?>" class="space-y-4">
                    <?php echo csrf_field(); ?>
                    <?php echo method_field('PATCH'); ?>

                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700">Nama Lengkap</label>
                        <input type="text" name="name" value="<?php echo e(old('name', Auth::user()->name)); ?>" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>

                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                        <input type="email" name="email" value="<?php echo e(old('email', Auth::user()->email)); ?>" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\sicerdas\resources\views/profile/edit.blade.php ENDPATH**/ ?>