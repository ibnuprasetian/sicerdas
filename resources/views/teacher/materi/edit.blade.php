        ...
        <form action="{{ route('teacher.materi.update', $materi->id) }}" method="POST"
            ...
            <div class="mb-4">
                ...
                <input type="text" name="judul" id="judul" class="form-control form-control-lg rounded-pill"
                    value="{{ $materi->judul }}" required placeholder="Masukkan judul materi">
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="deskripsi" id="deskripsi" class="form-control rounded-lg" rows="3"
                    placeholder="Jelaskan materi secara singkat">{{ $materi->deskripsi }}</textarea>
            </div>
                ...
                @foreach($materi->subMateris as $index => $sub)
                    <div class="submateri-group mb-4 p-3 rounded-lg bg-light border border-gray-200 animate__animated animate__slideInLeft"
                        ...
                        <div class="row g-3 align-items-center">
                            <div class="col-md-6">
                                <label for="sub_judul_{{ $index }}" class="form-label fw-semibold"><i
                                        class="bi bi-textarea-resize me-2 text-warning"></i> Judul Sub Materi <span
                                        class="text-danger">*</span></label>
                                <input type="text" name="sub_judul[]" id="sub_judul_{{ $index }}" value="{{ $sub->judul }}"
                                    class="form-control rounded-pill" placeholder="Judul Sub Materi" required>
                            </div>
                            <div class="col-md-6">
                                <label for="youtube_link_{{ $index }}" class="form-label fw-semibold"><i
                                        class="bi bi-youtube me-2 text-danger"></i> Link YouTube <span
                                        class="text-danger">*</span></label>
                                <input type="text" name="youtube_link[]" id="youtube_link_{{ $index }}"
                                    value="{{ $sub->youtube_link }}" class="form-control rounded-pill"
                                    placeholder="Link YouTube" required>
                            </div>
                            <div class="col-12">
                                <label for="sub_deskripsi_{{ $index }}" class="form-label fw-semibold"><i
                                        class="bi bi-paragraph me-2 text-info"></i> Deskripsi Sub Materi</label>
                                <textarea name="sub_deskripsi[]" id="sub_deskripsi_{{ $index }}" class="form-control rounded-lg"
                                    placeholder="Deskripsi Sub Materi" rows="2">{{ $sub->deskripsi }}</textarea>
                            </div>
                            ...
                            <input type="hidden" name="old_sub_id[]" value="{{ $sub->id }}">
                        </div>
                    </div>
                @endforeach
            </div>
