    ...
    <div class="card shadow-lg border-0 rounded-4 p-5 animate__animated animate__fadeIn">
        <div class="text-center">
            ...
            <p class="fs-5 mt-3 animate__animated animate__fadeInUp" style="animation-delay: 0.2s;">
                Selamat, kamu telah menaklukkan Level <strong class="text-primary">{{ $level }}</strong> dalam petualangan yang luar biasa ini! <i class="bi bi-trophy-fill text-warning ms-2"></i>
            </p>
        </div>

        <div class="text-center my-4 animate__animated animate__fadeIn" style="animation-delay: 0.4s;">
            <h4 class="text-info"><i class="bi bi-star-fill text-warning me-2"></i> Total XP yang Kamu Raih: <span class="fw-bold text-success">{{ $skor }}</span></h4>
            @if(isset($xp_gain) && $xp_gain > 0)
                <p class="text-success"><i class="bi bi-sparkles text-warning me-2"></i> Bonus XP untuk jawaban yang tepat: <strong class="text-info">+{{ $xp_gain }}</strong></p>
            @endif
        </div>
        ...
        <div class="d-flex justify-content-center gap-3 flex-wrap mt-4 animate__animated animate__slideInUp" style="animation-delay: 0.6s;">
            <form method="POST" action="{{ route('quiz.petualangan.submitLevel') }}" class="animate__animated animate__pulse animate__infinite infinite">
                @csrf
            ...
            <a href="{{ route('quiz.petualangan.reset') }}" class="btn btn-warning btn-lg rounded-pill shadow-sm animate__animated animate__shakeX" style="animation-delay: 0.8s;">
